...
class ReflectionWorker(AgentWorkerBase):
    ...
    def __init__(self, connection_params, stop_event):
        ...
        self.gateway = init_llm_gateway("reflection")
    ...
    def handle_task(self, task: TaskMessage) -> ResultMessage:
        ...
        system = (
            ...
            "If attempt_history is provided, compare the current failure signature against prior attempts and "
            "explicitly call out repeated signatures, stale patch strategies, and what must change this attempt. "
            "If stuck=true, prioritize differentiating checks that can falsify repeated assumptions quickly. "
        ...
        user = (
            ...
            f"Attempt history (oldest->newest, compact):\n{json.dumps(ctx.get('attempt_history', []), indent=2)}\n"
            f"Stagnation flags:\n"
            f"- stuck: {bool(ctx.get('stuck', False))}\n"
            f"- stuck_reason: {ctx.get('stuck_reason', '')}\n"
