...
| `ACCEPTANCE = "AcceptanceWorker"`     | Acceptance gating (artifacts/metrics)          |
...
Tier‑1 planning artifacts are modeled in `core/schemas/specifications.py`. They extend a shared `SpecificationDocument` base (metadata such as `spec_id`, `state`, `revision`, `created_by`, `approved_by`, content hash, and upstream references). Each level adds its own payload:
...
| `L2Specification` | Interface contract               | `clocking` (list of `ClockingInfo` entries with clock/reset polarity; may be empty for purely combinational interfaces), `signals` (direction + width expression), `handshake_semantics`, `transaction_unit`, `configuration_parameters` |
...
| `L4Specification` | Architecture / microarchitecture | `block_diagram` nodes, `dependencies`, `connections`, `clock_domains`, `resource_strategy`, `latency_budget`, `assertion_plan`                                                          |
