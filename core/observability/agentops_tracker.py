...
- Sends PER-CALL LLM EVENTS to AgentOps (tokens, cost, agent, node, metadata)
...
class AgentOpsTracker:
    def __init__(self) -> None:
        ...
        self._otel_context = None  # OpenTelemetry context for worker-thread propagation
    ...
    def get_otel_context(self):
        """Return the OpenTelemetry context for propagation to worker threads."""
    ...
    def init_from_env(
        ...
        try:
            ...
            self._trace_ctx = agentops.start_trace(
                trace_name=self.run_name, tags=tags
            )
            ...
            print(f"DEBUG: AgentOps init failed: {type(e).__name__}: {e}")
            import traceback
    ...
    # -------------------------------
    # NEW: Emit per-call AgentOps event
    # -------------------------------
    def _emit_agentops_llm_event(self, entry: Dict[str, Any]) -> None:
        if not self.enabled or agentops is None:
            return
        try:
            agentops.update_trace_metadata(
                {"last_llm_call": json.dumps(entry)}
            )
        except Exception:
            pass
    ...
    def log_llm_call(
        ...
        *,
        ...
        # SEND PER-CALL EVENT TO AGENTOPS
        self._emit_agentops_llm_event(entry)
        ...
        with self._lock:
            ...
            self._totals["estimated_cost_usd"] += float(
                ...
                agentops.update_trace_metadata(
                    {
                        ...
                        "total_completion_tokens": self._totals[
                            "completion_tokens"
                        ],
                        "total_tokens": self._totals["total_tokens"],
                        "total_estimated_cost_usd": round(
                            self._totals["estimated_cost_usd"], 6
                        ),
                    }
                )
            except Exception:
    ...
    def log_event(self, event_type: str, payload: Dict[str, Any]) -> None:
        ...
        try:
            agentops.update_trace_metadata(
                {f"last_event.{event_type}": json.dumps(payload)}
            )
        except Exception:
            pass
