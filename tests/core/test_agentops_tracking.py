...
def test_log_llm_call_skips_manual_openai_span_when_auto_instrumentation_enabled(tmp_path, monkeypatch):
    monkeypatch.setattr(tracker_mod, "ARTIFACTS_DIR", tmp_path)
    tracker = tracker_mod.AgentOpsTracker()
    tracker.enabled = True
    tracker.run_id = "run-1"
    tracker._openai_auto_llm_instrumentation = True
    called = {"count": 0}
    monkeypatch.setattr(tracker, "_record_span", lambda **kwargs: called.__setitem__("count", called["count"] + 1))
...
def test_task_span_uses_readable_parent_span_names(monkeypatch):
    tracker = tracker_mod.AgentOpsTracker()
    tracker.enabled = True
    recorded: dict = {}

    @contextmanager
    def _fake_start_current_span(**kwargs):
        recorded.update(kwargs)
        yield

    monkeypatch.setattr(tracker, "start_current_span", _fake_start_current_span)

    with tracker.task_span(
        task_type="ImplementationAgent",
        node_id="buf1_leaf",
        run_id="run-1",
        task_id="task-1",
        attempt=2,
        runtime="agent_implementation",
    ):
        pass

    assert recorded["operation_name"] == "implementation.buf1_leaf.attempt2"
    assert recorded["span_kind"] == SpanKind.AGENT
    assert recorded["attributes"][AgentAttributes.AGENT_NAME] == "implementation"
    assert recorded["attributes"]["mhd.task_type"] == "ImplementationAgent"
    assert recorded["attributes"]["mhd.runtime"] == "agent_implementation"
...
def test_init_enables_agentops_openai_auto_instrumentation_when_beta_chat_present(monkeypatch):
    class _FakeAgentOps:
        def __init__(self):
            self.init_kwargs = None
    ...
    monkeypatch.setattr(
        tracker_mod,
        "get_runtime_config",
        lambda: SimpleNamespace(llm=SimpleNamespace(provider="openai", default_model="gpt-4.1-mini")),
    )
    monkeypatch.setattr(tracker_mod.importlib.util, "find_spec", lambda name: object())
    monkeypatch.setenv("AGENTOPS_API_KEY", "test-key")
    ...
    assert tracker.enabled is True
    assert fake_agentops.init_kwargs is not None
    assert fake_agentops.init_kwargs["instrument_llm_calls"] is True
    assert tracker._openai_auto_llm_instrumentation is True
